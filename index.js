...
function transformModule(inputFilename, outputFilename, transformFn) {
  ...
  function isRelativePathWithoutExtension(path) {
    if (!path) {
      // null, undefの場合
      return false;
    }
    // 文字列が "/" または "./" または "../" で始まるかどうかをチェックする
    if (!/^(\.?\.?\/)/.test(path)) {
      return false;
    }

    // 拡張子がないかどうかをチェックする
    return path.indexOf(".", path.lastIndexOf("/")) === -1;
  }
  ...
  function handleImportExportDeclaration(path, self) {
    ...
    if (isRelativePathWithoutExtension(sourceValue)) {
